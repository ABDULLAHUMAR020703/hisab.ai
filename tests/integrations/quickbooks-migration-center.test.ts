...
test('dashboard restoration derives overview from hydrated session only', () => {
  ...
  assert.equal(view.workerStatus, 'running')
  ...
  assert.match(page, /\/api\/import-export\/migration-sessions\/\$\{sessionId\}/)
...
test('wizard is configuration-only after migration starts', () => {
  ...
  assert.match(provider, /router\.push\(migrationCenterPath/)
