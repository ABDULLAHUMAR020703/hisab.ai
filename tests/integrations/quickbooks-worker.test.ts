...
test('standalone QuickBooks worker polls only QuickBooks migration jobs (import + snapshot steps)', () => {
  ...
  assert.match(worker, /processJobBatch\(1, \['QUICKBOOKS_IMPORT_STEP', 'QUICKBOOKS_SNAPSHOT_STEP'\]\)/)
  assert.match(worker, /while \(!stopping\)/)
  assert.match(workers, /processJobBatch\(limit = 5, jobType\?: string \| string\[\]\)/)
  assert.match(queue, /query = query\.eq\('job_type', jobType\)/)
