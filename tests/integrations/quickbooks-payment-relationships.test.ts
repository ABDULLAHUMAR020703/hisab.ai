...
test('QuickBooks zero-cash linking payments pair standalone credit lines to invoices',()=>{const result=extractQuickBooksPaymentRelationships({TotalAmt:0,UnappliedAmt:0,Line:[linked(100,'Invoice','INV-1'),linked(100,'CreditMemo','CM-7')]},'CUSTOMER');assert.equal(result.issues.length,0);assert.equal(result.allocations[0].cashAmount,0);assert.equal(result.allocations[0].creditAmount,100);assert.deepEqual(result.allocations[0].creditSourceIds,['CM-7'])})
