...
export function extractQuickBooksDepositRelationships(raw:Row):QuickBooksDepositRelationships{
  ...
  const cashBack=object(raw.CashBack),cashBackAccount=object(cashBack.AccountRef),cashBackAmount=round(number(cashBack.Amount))
  if(cashBackAmount>0){const accountId=String(cashBackAccount.value??'');if(accountId)allocations.push({sourceLineKey:`cashback:Account:${accountId}`,amount:-cashBackAmount,sourceTransactionType:'Account',sourceTransactionId:null,sourceAccountId:accountId,sourceEntityId:null,description:String(cashBack.Memo??'QuickBooks deposit cash back'),metadata:{cashBack:true}});else issues.push('QuickBooks deposit cash back has no source account.')}
