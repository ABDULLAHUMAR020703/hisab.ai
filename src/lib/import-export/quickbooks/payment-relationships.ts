...
export function extractQuickBooksPaymentRelationships(raw:Row,kind:QuickBooksPaymentKind):QuickBooksPaymentRelationships {
  ...
  for(const allocation of allocations){if(!allocation.creditSourceIds.length)continue;allocation.creditAmount=Math.min(allocation.amount,creditRemaining);allocation.cashAmount=round(allocation.amount-allocation.creditAmount);creditRemaining=round(creditRemaining-allocation.creditAmount)}
