      ...
      <div className="space-y-5">
        ...
        {!showBlockedGate && step === 'analyze' && (
          <div className="space-y-5">
            ...
            {source?.connected && <>
              <dl className="grid grid-cols-2 gap-4 rounded-xl bg-slate-50 p-4 text-sm md:grid-cols-6">
                <div><dt className="text-xs text-slate-400">Company</dt><dd className="mt-1 font-medium text-slate-800">{analysis?.companyName ?? source.companyName ?? 'Connected company'}</dd></div>
                <div><dt className="text-xs text-slate-400">Environment</dt><dd className="mt-1 font-medium text-slate-800">{(analysis?.environment ?? source.environment) === 'production' ? 'Production' : (analysis?.environment ?? source.environment) === 'sandbox' || source.connected ? 'Sandbox' : '—'}</dd></div>
                <div><dt className="text-xs text-slate-400">Fiscal year</dt><dd className="mt-1 font-medium text-slate-800">{analysis?.fiscalYear ?? 'Not provided'}</dd></div>
                <div><dt className="text-xs text-slate-400">Country</dt><dd className="mt-1 font-medium text-slate-800">{analysis?.country ?? source.country ?? '—'}</dd></div>
                <div><dt className="text-xs text-slate-400">Currency</dt><dd className="mt-1 font-medium text-slate-800">{analysis?.currency ?? source.baseCurrency ?? '—'}</dd></div>
                <div><dt className="text-xs text-slate-400">Estimated time</dt><dd className="mt-1 font-medium text-slate-800">{analysis ? `~${analysis.estimatedMigrationMinutes} min` : 'Calculating…'}</dd></div>
              </dl>
              {(source.environmentMismatch || (source.serverEnvironment === 'production' && (source.environment ?? 'sandbox') !== 'production')) && (
                <div role="alert" className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                  QuickBooks is connected to a Sandbox company. Reconnect a Production QuickBooks company in Settings before migrating.
                </div>
              )}
        ...
        {!showBlockedGate && step === 'import' && <div className="space-y-5">{loading ? <div className="py-12 text-center"><RefreshCw className="mx-auto mb-4 animate-spin text-indigo-500" /><p className="text-sm text-slate-600">Starting Migration Center…</p></div> : <><div className="flex items-start gap-3 rounded-xl border border-indigo-100 bg-indigo-50 p-4"><ShieldCheck size={20} className="mt-0.5 text-indigo-600" /><div><h3 className="font-semibold text-indigo-900">Ready to migrate</h3><p className="mt-1 text-sm text-indigo-800">QuickBooks Online<br />Environment: {(analysis?.environment ?? source?.environment) === 'production' ? 'Production' : 'Sandbox'}<br />Company: {analysis?.companyName ?? source?.companyName ?? 'Connected company'}{analysis?.realmId || source?.realmId ? <> · Realm {analysis?.realmId ?? source?.realmId}</> : null}<br /><br />Your data will be imported from this QuickBooks {(analysis?.environment ?? source?.environment) === 'production' ? 'production' : 'sandbox'} company. {sourceRecordCount.toLocaleString()} source records will be extracted in the background. Preview samples are informational and are not reused. Select how existing records should be handled. After you start, progress continues in the Migration Center.</p></div></div><DuplicateStep duplicateCount={duplicateCount} strategy={strategy} onStrategyChange={setStrategy} /></>}<ModuleLifecycleList modules={lifecycleModules} onRetryPreview={(key) => void retryPreview(key)} busy={loading} /></div>}
