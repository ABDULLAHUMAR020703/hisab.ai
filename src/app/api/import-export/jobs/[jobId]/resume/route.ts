...
export async function POST(_request: Request, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    return importModule(new Request('http://internal/import', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...payload, jobId }) }), { params: Promise.resolve({ module: job.moduleKey }) })
