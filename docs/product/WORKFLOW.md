...
| Doc | When to read |
|-----|----------------|
| [CONTEXT.md](./CONTEXT.md) | Feature inventory, gaps, data model overview |
| [SUPABASE.md](../migration/SUPABASE.md) | Only if switching runtime DB to Postgres |
...
**Agent caution:** Data written on Vercel is **ephemeral** per instance unless you migrate to Postgres (see `docs/migration/SUPABASE.md`). Do not assume persistent SQLite on serverless.
...
**Supabase path:** If `datasource` changes to `postgresql`, follow `docs/migration/SUPABASE.md` — do not mix SQLite adapter code with Postgres without refactoring `src/lib/prisma.ts`.
...
| Area | Doc | API |
|------|-----|-----|
| XML | `docs/testing/ZATCA_TESTING_GUIDE.md` | `GET /api/zatca/invoices/[id]/xml` |
| Hash/QR | same | `/hash`, `/qr` |
| Onboarding | `docs/zatca/CLIENT_ONBOARDING_FORM.md` | `/api/zatca/onboarding/*`, `/api/zatca/onboard` |
| Submission | `docs/zatca/PRODUCTION_READINESS.md` | `/submit`, `/status` |
| E2E / prod | `docs/zatca/RELEASE_CHECKLIST.md` | `/sandbox/run` |
...
Moving to Supabase/Postgres?
  YES → docs/migration/SUPABASE.md + remove better-sqlite3 adapter from prisma.ts
```
